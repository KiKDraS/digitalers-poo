      ...
      return {
        ...
        next: () => {
          ...
          return {
            ...
            done: typeof this.arr[this.index + 1] !== "undefined",
          };
        },
        current: () => this.arr[this.index],
        //Opcionales
        hasNext: () => {
          return typeof this.arr[this.index + 1] !== "undefined";
        },
...
recorrerGaleria = ({ target: { classList } }) => {
  const src = imgCentral.getAttribute("src");

  if (classList.contains("flechaIzquierda")) {
    //console.log(iterable.hasPrev());
    if (iterable.hasPrev()) {
      iterable.back();
      imgCentral.setAttribute("src", iterable.current());
    } else {
      iterable.goLast();
      console.log("prev first", iterable.current());
      imgCentral.setAttribute("src", iterable.current());
    }
  } else {
    if (iterable.hasNext()) {
